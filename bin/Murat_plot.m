...
y                                   =   Murat.input.y;
z                                   =   Murat.input.z;
Murat.input.sections                =   [64.11 -21.37 -4500];
...
%for k = 1
   ...
   % saveas(rays_peakDelay,pathFolder);
   ...
   % close(rays_Q)
    ...
    %hold on
    %faults2= shaperead('FagliePollino.shp');  %load the shapefile in your folder
    %mapshow(faults2, 'Color','black', 'LineWidth',1.5) %read the shapefile
    %hold on    
    coast = shaperead('Coastline.shp');
    mapshow(coast,'Facealpha',0, 'LineWidth',1.4)
...
%     hold on      
    ...
    [Qcmap,qc_inter,xi,yi,zi,Xi,Yi,Zi]...
        ...
        hot,sections,evestaz_Qc,x,y,z,divi,FName_QcMap);
    hold on
%     faults= shaperead('Faglie_LatLon.shp');  %load the shapefile in your folder
%     mapshow(faults, 'Color',[0.8 0.8 0.8], 'LineWidth',1.5) %read the shapefile
%     hold on  
%     faults2= shaperead('FagliePollino.shp');  %load the shapefile in your folder
%     mapshow(faults2, 'Color',[0.8 0.8 0.8], 'LineWidth',1.5) %read the shapefile
%     hold on     
    coast = shaperead('Coastline.shp');
    mapshow(coast,'Facealpha',0, 'EdgeColor', [0.8 0.8 0.8], 'LineWidth',1.4)
    xlim([origin(2) ending(2)]); %if necessary, cut the shapefile in your area of interest
    ylim([origin(1) ending(1)]);
    hold off
